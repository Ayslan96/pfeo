...
public:
    ...
    friend std::ostream& operator << (std::ostream &saida, const Animal& p){
    	saida << "Id: " << p.m_id << " Classe: " << p.m_class<< " Nome cientifico: " << p.m_nome_cientifico << " Sexo: " << p.m_nome_cientifico << " Tamanho: " 
    	<< p.m_tamanho << " Dieta: " << p.m_dieta << " Nome de batismo: " << p.m_nome_batismo;
		return saida;
	}
